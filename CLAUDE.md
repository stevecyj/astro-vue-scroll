...
- **Stagewise Toolbar**: Development tool for debugging, integrated via LayoutWrapper.vue
- **Multi-Instance Protection**: Uses `lodash-es.once()` in `src/modules/app.js` to prevent duplicate Vue instances in Astro Island architecture
...
**結構完整性檢查**:
- ✅ 所有 6 個頁面檔案正確實作
- ✅ Vue 組件與 Astro 頁面完美對應  
- ✅ 頁面類型分類 (special/normal) 正確設定
- ✅ 動態轉場系統完整實作
- ✅ Multi-instance protection 機制運作正常
- ✅ Stagewise Toolbar 透過 LayoutWrapper.vue 統一整合
